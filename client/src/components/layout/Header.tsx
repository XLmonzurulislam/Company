        ...
        <nav className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            {isLoading ? (
              ...
              <DropdownMenu>
                  ...
                  <Button variant="ghost" className="relative h-10 w-10 rounded-full">
                    <Avatar className="h-9 w-9 border border-secondary">
                      <AvatarFallback className="bg-primary-foreground text-secondary">
                        {user.fullName?.split(' ').map((n: string) => n[0]).join('') || user.username?.substring(0, 2).toUpperCase() || 'U'}
                      </AvatarFallback>
                    </Avatar>
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent className="w-56" align="end">
                  ...
                  <DropdownMenuItem asChild>
                    <Link 
                      href={user.userType === "client" ? "/client/dashboard" : "/hacker/profile"} 
                      className="cursor-pointer flex w-full items-center"
                    >
                      <User className="mr-2 h-4 w-4" />
                      <span>{user.userType === "client" ? "Dashboard" : "My Profile"}</span>
                    </Link>
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
              ...
              <>
                <Link href="/login" className="text-light-text hover:text-secondary transition-colors hidden md:inline-block">
                  Login
                </Link>
                <Link href="/signup">
